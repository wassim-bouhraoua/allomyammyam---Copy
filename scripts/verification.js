// verification.js – minimal checkout verification
...
(async () => {
  ...
  // Create two chefs (same city)
  const chefAUser = await prisma.user.create({ data: { email: 'chefA@test.com', password: 'pw', firstName: 'Chef', lastName: 'A', role: 'CHEF', city: 'TestCity' } });
  const chefBUser = await prisma.user.create({ data: { email: 'chefB@test.com', password: 'pw', firstName: 'Chef', lastName: 'B', role: 'CHEF', city: 'TestCity' } });
  ...
  // Dishes with stock
  const dishA = await prisma.dish.create({ data: { chefId: chefA.id, name: 'Dish A', price: new Decimal(10), category: 'MAIN_COURSE', stockCount: 5, preparationTime: 15 } });
  const dishB = await prisma.dish.create({ data: { chefId: chefB.id, name: 'Dish B', price: new Decimal(20), category: 'MAIN_COURSE', stockCount: 3, preparationTime: 20 } });

  // Cart for customer with items from both chefs
